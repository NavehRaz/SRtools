...
def baysianDistance2(sr1, sr2, time_range=None, dt=1, debug=False):
    """
    Calculate the likelihood that the death times of sr1 are generated by sr2.
    This version uses the empirical CDF of sr2 (no smoothing).
    Only uses deathtimes from sr1 that are within the range of the minimum and maximum deathtimes in sr2.
    """
    ...
    # Only consider those who died in sr2
    died2 = death_times2[events2 == 1]
    n2 = len(events2)
    n2_died = len(died2)

    # If not enough events, return -inf
    if n2 <= 5 or n2_died <= 5:
        return -np.inf
    ...
    # Restrict to time_range if provided
    if time_range is not None:
        mask1 = (death_times >= time_range[0]) & (death_times <= time_range[1])
        events = events[mask1]
        dts = dts[mask1]
        death_times = death_times[mask1]

    # Further restrict sr1 to only those deathtimes within the min/max of sr2's deathtimes
    if len(death_times2) > 0:
        min_dt2 = np.min(death_times2)
        max_dt2 = np.max(death_times2)
        mask_range = (death_times >= min_dt2) & (death_times <= max_dt2)
        events = events[mask_range]
        dts = dts[mask_range]
        death_times = death_times[mask_range]

    died = death_times[events == 1]
    censored = death_times[events == 0]
    ndied = len(died)
    p_death_before_t_end = n2_died / n2

    # Sort died2 for efficient search
    died2_sorted = np.sort(died2)
    n_died2 = len(died2_sorted)

    # For each death in sr1, compute log probability using empirical PDF
    # Use adaptive binning: assign to nearest bin in sr2 to avoid gaps
    # p(t) = (number of deaths in bin around nearest sr2 death) / n2_died / dt
    logps = 0.0
    for t, dt_i in zip(died, dts[events == 1]):
        # Find the nearest death time in sr2
        nearest_idx = np.argmin(np.abs(died2_sorted - t))
        nearest_death_time = died2_sorted[nearest_idx]
        
        # Use the nearest death time as the center of our bin
        t_center = nearest_death_time
        t0 = t_center - dt_i/2
        t1 = t_center + dt_i/2
        
        # Use searchsorted for fast bin counting
        left = np.searchsorted(died2_sorted, t0, side='left')
        right = np.searchsorted(died2_sorted, t1, side='right')
        count = right - left
        
        # Avoid log(0)
        if count == 0 or n2_died == 0 or dt_i == 0:
            if debug:
                print(f"Zero count for t={t}, dt={dt_i}, nearest={nearest_death_time}, left={left}, right={right}")
            return -np.inf
        logps += np.log(count) - np.log(n2_died) - np.log(dt_i)

    # For censored, use empirical survival function S(t) = 1 - CDF(t)
    # S(t) = (number of deaths > t) / n2_died
    logps_censored = 0.0
    for t in censored:
        right = np.searchsorted(died2_sorted, t, side='right')
        n_survive = n_died2 - right
        if n_survive <= 0 or n2_died == 0:
            # If all simulated died before t, S(t) = 0, log(0) = -inf
            if debug:
                print(f"Zero survival for censored t={t}, right={right}")
            return -np.inf
        logps_censored += np.log(n_survive) - np.log(n2_died)

    # Correction for conditioning on death before t_end
    if p_death_before_t_end <= 0 or p_death_before_t_end > 1:
        if debug:
            print(f"Invalid p_death_before_t_end: {p_death_before_t_end}")
        return -np.inf
